...
Su 主题使用 `scripts/audit-su-contrast.py` 计算 WCAG 2.2 对比度：

```bash
scripts/audit-su-contrast.py
```

审计规则：

- WCAG 2.2 AA 正文：对比度 `>= 4.5:1`
- WCAG 2.2 AA 大字与非文本 UI：对比度 `>= 3:1`
- 计算时不四舍五入，`4.499:1` 仍视为不通过
- 额外检查关键语法色 RGB 距离 `>= 45`，这是 Su 的可辨性约束，不是 WCAG 条款
